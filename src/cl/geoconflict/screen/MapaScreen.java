...
public class MapaScreen extends Screen {
	...
	public MapaScreen(Game game, Client client, GameStates gamestates) {
		...
		this.map = new Map( this.game.getGraphics().getHeight(), this.game.getGraphics().getWidth() );
		
		// se crean los sprites
		Log.debug("Posiciones: " + this.gameStates.getPositions().size());
		this.playerPoints = new PlayerPoint[this.gameStates.getPositions().size()];
		Enumeration<String> enumKey = this.gameStates.getPositions().keys();
		int cont = 0;
		while(enumKey.hasMoreElements()) {
			// Las posiciones se identifican con el nombre de usuario del jugador
			String key = enumKey.nextElement();
			this.playerPoints[cont] = new PlayerPoint(Assets.playerWhite, key, 20, 20, 1, 6);
			Position pos = this.map.getPosition(
					this.gameStates.getPositions().get(key)[0],
					this.gameStates.getPositions().get(key)[1]	);
			this.playerPoints[cont++].setPosition(
					(int) pos.x, 
					(int) pos.y);
			Log.debug("Posiciones: (" + this.gameStates.getPositions().get(key)[0] 
					+ ", " + this.gameStates.getPositions().get(key)[1] + " )");
		}
	...
	public void update(float deltaTime) {
		...
		if( this.gameStates.newPosition ){
			Enumeration<String> enumKey = this.gameStates.getPositions().keys();
			while(enumKey.hasMoreElements()) {
				// Las posiciones se identifican con el nombre de usuario del jugador
				String key = enumKey.nextElement();
			...
			this.gameStates.newPosition = false;
		}
	...
	public void present(float deltaTime) {
        ...
        g.drawTextRotate("dir:" + this.player.getDirection(), 150, 140, Color.GREEN,20, 90);
      	...
      	//mostrar jugadores
      	for(int i = 0; i < this.gameStates.getPositions().size() ; i++)
